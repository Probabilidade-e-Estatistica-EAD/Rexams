...
questions[1] <- "A localização de ambas as distribuições é aproximadamente a mesma."
solutions[1] <- abs(trob(A, B)) < 0.5
explanations[1] <- if (solutions[1]) "Ambas as distribuições têm uma localização semelhante." else
  paste("Distribuição ", c("A", "B")[(median(A) < median(B)) + 1],
    " tem em média valores mais altos que a distribuição ", 
    c("A", "B")[(median(A) >= median(B)) + 1], ".", sep = "")

outlier <- length(unlist(outlier)) > 0
questions[2] <- "Ambas as distribuições não contêm valores discrepantes."
solutions[2] <- !outlier
explanations[2] <- if (solutions[2])
  "Ambas as distribuições não têm observações que desviem mais de 1,5 vezes a amplitude interquartil." else "Existem observações que diferem mais de 1,5 vezes a amplitude interquartil."

questions[3] <- "A dispersão na amostra A é maior que na B."
solutions[3] <- IQR(A)/IQR(B) > 1.5
explanations[3] <- paste("A amplitude interquartil da amostra A",
  ifelse(solutions[3], "", "_não_"), "é maior que em B.")

questions[4] <- "A simetria de ambas as amostras é semelhante."
solutions[4] <- SK_A == SK_B
explanations[4] <- if (solutions[4]) paste("A assimetria de ambas as distribuições é semelhante, ambas são",
    ifelse(abs(SK(A)) < 0.2, "simétrica.", ifelse(SK(A) >= 0.2, "assimétrica à direita.", "assimétrica à esquerda."))) else
    paste("A simetria de ambas as distribuições é diferente. Amostra A é",
    ifelse(abs(SK(A)) < 0.2, "simétrica..", ifelse(SK(A) >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")),
    "Amostra B é", ifelse(abs(SK(B)) < 0.2, "simétrica.", ifelse(SK(B) >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")))

i <- sample(1:2, 1)
j <- sample(1:3, 1)
questions[5] <- paste("Distribuição ", c("A", "B")[i], " é ", 
...
explanations[5] <- paste("Distribuição ", c("A", "B")[i], " é ",
                         ifelse(abs(SK_i) < 0.2, "simétrica.", 
                                ifelse(SK_i >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")))
```

Question
========
Na figura seguinte são dadas as distribuições de uma variável em duas amostras (A e B), representadas pelos boxplots verticais. Considerando os gráficos apresentados, quais das seguintes afirmações são corretas (mais de uma pode estar correta - marque todas as corretas)?
\
